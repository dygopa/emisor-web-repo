...
function ToEmmitPolicyRegister() {
  ...
  const [policeAnualPrimeContractor, setPoliceAnualPrimeContractor] =
    useState(false);
  ...
  const [dateBirthClient, setDateBirthClient] = useState(
    moment().subtract(16, "y").format("YYYY-MM-DD")
  );
  const [dateBirthClientContractor, setDateBirthClientContractor] = useState(
    moment().subtract(16, "y").format("YYYY-MM-DD")
  );
  ...
  const [errorMessage, setErrorMessage] = useState(
    "Ocurrio un error, intentelo otra vez "
  );
  ...
  const [listOfTypeDocumentContractor, setListOfTypeDocumentContractor] =
    useState([]);
    ...
    return (
      <div
        onClick={() => {
          ...
          executeScroll(ownRef);
        }}
        className={`cursor-pointer flex w-full h-auto mb-3 rounded p-2 justify-start items-center ${
          activeLink === number
      ...
      >
        <p
          className={`mr-3 font-light flex w-[35px] h-[35px] rounded-full justify-center items-center text-lg border-2 ${
            activeLink === number
        ...
        <p
          className={`text-sm text-secondary ${
            activeLink === number ? "font-bold" : "font-light"
          }`}
        >
          {title}
        </p>
  ...
  function handleFindIDInformation() {
    if (formObject["numberId"] !== undefined) {
      ...
      apiProvider
        .GetDatosIdentidadxIdentificacionEndPoint(query)
        .then((res) => {
          let listRes = [...res.data];
          if (listRes.length > 0) {
            let dateBirth = moment(listRes[0]["FechaNacimiento"]).format(
              "YYYY-MM-DD"
            );
            ...
            formObject.IdDistrito =
              parseInt(json["IdDistritoIntegracion"]) ?? "";
            formObject.IdCorregimiento =
              parseInt(json["IdCorregimientoIntegracion"]) ?? "";
            ...
            formObject.NombreConductorAdicional =
              json["NombreConductorAdicional"] ?? "";
            formObject.ApellidoConductorAdic =
              json["ApellidoConductorAdic"] ?? "";
  ...
  function handleFormatObjectFromAPI(json) {
    ...
    apiProvider
      .GetDatosContratanteEndPoint(query)
      .then((res) => {
        if (res.data[0]) {
          let dateBirth = moment(res.data[0]["FechaNacimiento"])
            .format("YYYY-MM-DD")
            .toString();
  ...
  function getDataFromValue(value, type) {
    if (type === "model") {
      apiProvider
        .getVehiculeModelEndPoint(
          `?idMarca=${value}&idCompania=${data.state["idCompania"]}`
        )
        .then((res) => {
          console.log(value);
    ...
    if (type === "provincia") {
      apiProvider
        .getManzaneroEndPoint(`?Tipo=d&IdProvincia=${value}`)
        .then((res) => {
    ...
    if (type === "distrito") {
      apiProvider
        .getManzaneroEndPoint(
          `?Tipo=c&IdProvincia=${formObject["IdProvincia"]}&IdDistrito=${value}`
        )
        .then((res) => {
    ...
    if (type === "corregimiento") {
      apiProvider
        .getManzaneroEndPoint(
          `?Tipo=u&&IdProvincia=${formObject["IdProvincia"]}&&IdDistrito=${formObject["IdDistrito"]}&CodCorredimiento=${value}`
        )
        .then((res) => {
  ...
  function chargeAPI() {
    ...
    apiProvider
      .getVehiculeBrandEndPoint(`?idCompania=${data.state["idCompania"]}`)
      .then((res) => {
        ...
        <div className="max-w-[50%] w-fit min-w-[10%] h-fit min-h-[10vh]  top-[30%] bg-white border rounded-md overflow-hidden flex flex-col justify-between items-center gap-4 p-6 shadow-md">
          <div className="w-full flex flex-col justify-center items-center gap-4">
            <p className="font-semibold text-lg text-slate-900">
              ¿Está seguro que desea guardar los datos ingresados?
            </p>
            <p className="font-light text-base text-slate-900">
              Declaro y certifico bajo gravedad de juramento que la información
              declarada (datos personales, datos del auto, historial de
              conductor, datos del contratante), proveída y/o escrita es real,
              exacta, correcta y que no he omitido ningún hecho, circunstancia,
              dato o información relevante que hubieran podido influir de modo
              directo en la existencia o condiciones del contrato. El
              incumplimiento de la misma traen consigo la nulidad del mismo a
              criterio de la aseguradora correspondiente.
            </p>
          </div>
          <div className="w-full flex justify-end items-center gap-6">
            <div
              className="text-base text-slate-900 font-light cursor-pointer"
              onClick={() => {
                setTermsPopup(false);
              }}
            >
  ...
  const AlertComponent = ({ type, msg, state }) => {
    return (
      <div
        ...
        className={`transition shadow-md z-20 rounded p-[1%_2%] text-left h-auto w-auto cursor-pointer fixed right-[2.3%] bottom-[15%] block ${
          type === "1"
      ...
      >
        <p className={`font-semibold`}>
          {type === "1" ? "Exitos" : type === "2" ? "Error" : "Aviso"}
        </p>
        <p className={`font-light`}>
          {msg !== "" ? msg : "Mensaje de prueba para creacion de plan"}
        </p>
      </div>
    );
  };
  ...
  useEffect(() => {
    ...
    if (data.state["formObject"]) {
      ...
      setDateBirthClientContractor(
        data.state["formObject"]["dateBirthClientContractor"]
      );
      ...
      setPoliceAnualPrimeContractor(
        data.state["formObject"]["policeAnualPrimeContractor"]
      );
  ...
  return (
    <div
      className={`lg:ml-[6%] lg:w-[94%] w-full relative block h-screen bg-gray-50 p-8 ${
        termsPopup && "overflow-hidden"
      }`}
    >
      {termsPopup && <TermsPopup />}
      {warningStatus && (
        <AlertComponent
          state={setWarningStatus}
          type={"3"}
          msg={warningMessage}
        />
      )}
      {successStatus && (
        <AlertComponent
          state={setSuccessStatus}
          type={"1"}
          msg={successMessage}
        />
      )}
      {errorStatus && (
        <AlertComponent state={setErrorStatus} type={"2"} msg={errorMessage} />
      )}
      ...
      {navbarOnTop && !toggledSidebar && (
        <div className="shadow-[#7777772f] shadow-2xl transition z-20 fixed bg-white w-[94%] h-fit top-0 right-0">
          <div className="p-4 flex justify-between relative h-auto w-full items-center overflow-x-auto">
            <div className="w-fit flex justify-start items-center gap-3">
              ...
              <div className="h-10 relative block">
                <img
                  src={`./images/logos/logo-${data.state["imagen"]}`}
                  className="h-full box-border w-full object-contain"
                />
              </div>
              <div className="flex flex-col relative text-left justify-center items-left">
                <p className="font-light text-base text-slate-500">
                  {data.state["descripcion"]}
                </p>
                <p className="font-semibold text-base text-slate-900">
                  Total del plan:{" "}
                  <span className="text-xl">${data.state["totalPlan"]}</span>
                </p>
              </div>
            </div>
            <div
      ...
      {!toggledSidebar && (
        <div className="flex justify-between relative items-center h-fit w-full overflow-x-auto">
      ...
      <div className="flex w-full justify-between h-full">
        ...
        <div
          className={`mt-5 p-3 bg-white rounded-lg h-fit overflow-y-hidden ${
            !toggledSidebar ? "w-full" : "w-2/3"
          }`}
        >
          <div className="my-4 relative" ref={firstRef}>
            <p className={`title-section text-slate-900`}>
              Datos del Asegurado
            </p>
            <div className="flex flex-wrap content-start items-end">
              <div
                className={`${toggledSidebar ? "w-1/2" : "w-1/4"} mb-3 px-3`}
              >
                <p className="input-label">
                  Tipo Persona <span className="text-primary font-bold">*</span>
                </p>
                <div className="flex justify-between items-center">
                  ...
                  <p className="input-label flex items-center leading-[2px] mr-2">
                    ...
                    <div className="ml-3 w-1/4">
                      <div
                        onClick={() => {
                          setFormObject({ ...formObject, typePersona: "2" }),
                            chargeTypeOfDocument("2");
                        }}
                        className="group rounded-full w-5 h-5 border-solid border-[2.2px] p-[0.9px] box-border border-slate-500 overflow-hidden cursor-pointer flex justify-center items-center"
                      >
                        <span
                          className={`rounded-full transition w-full content-none h-full relative ${
                            formObject.typePersona === "2"
                              ? "group-hover:bg-slate-500 bg-primary"
                              : "group-hover:bg-secondary bg-slate-300"
                          }`}
                        ></span>
                      </div>
                    </div>
              ...
              >
                ...
                <select
                  defaultValue={formObject["idtipodocumento"]}
                  onChange={(e) => {
                    setFormObject({
                      ...formObject,
                ...
                >
                  ...
                  {listOfTypeDocument.map((type) => (
                    <option value={type["IdTipoDocumento"]}>
                      {type["TipoDocumento"]}
                    </option>
                  ))}
                </select>
              ...
              >
                ...
                <input
                  defaultValue={formObject["numberId"]}
                  placeholder="Ingrese el número de identificación"
                  onChange={(e) => {
                    setFormObject({ ...formObject, numberId: e.target.value });
                  }}
                  type="text"
                  className="form-control"
                />
            ...
            <div className="w-full h-fit flex flex-wrap content-start relative">
              {!loadedIDFromAPI && (
                <div className="w-full h-full absolute bg-white/40 backdrop-blur-sm bottom-0 left-0 z-10 flex flex-col justify-center items-center">
                  <AiFillLock size={70} color="#242424" />
                </div>
              )}

              {formObject["typePersona"] !== "1" && (
                <div
                  className={`${toggledSidebar ? "w-1/2" : "w-1/4"} mb-3 px-3`}
                >
                  <p className="input-label">Género</p>
                  <select
                    value={formObject["idsexo"]}
                    onChange={(e) => {
                      setFormObject({ ...formObject, idsexo: e.target.value });
                    }}
                    className="form-control"
                  >
                    <option value="">Seleccione el género</option>
                    {listOfGenre.map((type) => (
                      <option value={type["idSexo"]}>{type["sexo"]}</option>
                    ))}
                  </select>
              ...
              {formObject["typePersona"] !== "1" && (
                ...
                >
                  ...
                  >
                    ...
                    {listOfCivilStatus.map((type) => (
                      <option value={type["idEstadoCivil"]}>
                        {type["estadoCivil"]}
                      </option>
                    ))}
                  </select>
              ...
              {formObject["typePersona"] === "1" && (
                ...
                >
                  ...
                  <input
                    value={formObject["RepresentanteLegal"]}
                    ...
                    className="form-control"
                  />
                  {fieldsClientJuridic.includes("RepresentanteLegal") && (
                    <ErrorText />
                  )}
                </div>
              )}
              {formObject["typePersona"] === "1" && (
                ...
                >
                  ...
                  <input
                    value={formObject["ActividadComercialPrincipal"]}
                    placeholder="Ingrese la actividad comercial principal"
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        ActividadComercialPrincipal: e.target.value,
                      });
                    }}
                    type="text"
                    className="form-control"
                  />
                  {fieldsClientJuridic.includes(
                    "ActividadComercialPrincipal"
                  ) && <ErrorText />}
                </div>
              )}
              {formObject["typePersona"] === "1" && (
                ...
                >
                  ...
                  <input
                    value={formObject["NombreBeneficiario"]}
                    placeholder="Ingrese el nombre del beneficiario"
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        NombreBeneficiario: e.target.value,
                      });
                    }}
                    type="text"
                    className="form-control"
                  />
                  {fieldsClientJuridic.includes("NombreBeneficiario") && (
                    <ErrorText />
                  )}
                </div>
              )}
              {formObject["typePersona"] === "1" && (
                ...
                >
                  ...
                  <input
                    value={formObject["IdentificacionBeneficiario"]}
                    placeholder="Ingrese la identificacion del beneficiario"
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        IdentificacionBeneficiario: e.target.value,
                      });
                    }}
                    type="text"
                    className="form-control"
              ...
              {formObject["typePersona"] !== "1" && (
                ...
                >
                  ...
                  <input
                    value={formObject["names"]}
                    placeholder="Ingrese los nombres"
                    onChange={(e) => {
                      setFormObject({ ...formObject, names: e.target.value });
                    }}
                    type="text"
                    className="form-control"
                  />
              ...
              {formObject["idsexo"] === "1" &&
                formObject["typePersona"] !== "1" &&
                formObject["idestadocivil"] !== "1" && (
                  <div
                    className={`${
                      toggledSidebar ? "w-1/2" : "w-1/4"
                    } mb-3 px-3`}
                  >
                    <p className="input-label">Apellido de Casada</p>
                    <input
                      value={formObject["marriedLastName"]}
                      placeholder="Ingrese el apellido de casada"
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          marriedLastName: e.target.value,
                        });
                      }}
                      type="text"
                      className="form-control"
              ...
              {formObject["typePersona"] === "1" && (
                ...
                >
                  ...
                  <input
                    value={formObject["NombreComercial"]}
                    ...
                    className="form-control"
                  />
                  {fieldsClientJuridic.includes("NombreComercial") && (
                    <ErrorText />
                  )}
                </div>
              )}
              ...
              >
                ...
                <input
                  value={formObject["cellphone"]}
                  placeholder="Ingrese el número telefónico"
                  onChange={(e) => {
                    setFormObject({ ...formObject, cellphone: e.target.value });
                  }}
                  type="phone"
                  className="form-control"
                />
              ...
              {formObject["typePersona"] === "1" && (
                ...
                >
                  <p className="input-label">
                    País <span className="text-primary font-bold">*</span>
                  </p>
                  <select
                    value={formObject["PaisResidencia"]}
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        PaisResidencia: e.target.value,
                      });
                    }}
                    className="form-control"
                  >
                    <option value="">Seleccione el pais</option>
                    {listOfCountries.map((type) => (
                      <option value={type["IdPais"]}>{type["Pais"]}</option>
                    ))}
                  </select>
                  {fieldsClientJuridic.includes("PaisResidencia") && (
                    <ErrorText />
                  )}
                </div>
              )}
              {formObject["typePersona"] !== "1" && (
                ...
                >
                  ...
                  <select
                    defaultValue={formObject["IdPais"]}
                    value={formObject["IdPais"]}
                    onChange={(e) => {
                      setFormObject({ ...formObject, IdPais: e.target.value });
                    }}
                    className="form-control"
                  >
                    <option value="">Seleccione la nacionalidad</option>
                    {listOfNationalitys.map((type) => (
                      <option value={type["IdNacionalidad"]}>
                        {type["Nacionalidad"]}
                      </option>
                    ))}
                  </select>
              ...
              {formObject["typePersona"] !== "1" && (
                ...
                >
                  ...
                  <select
                    value={formObject["IdProfesion"]}
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        IdProfesion: e.target.value,
                      });
                    }}
                    className="form-control"
                  >
                    <option value="">Seleccione la profesión</option>
                    {listOfProfesion.map((type) => (
                      <option value={type["idprofesion"]}>
                        {type["profesion"]}
                      </option>
                    ))}
                  </select>
              ...
              {formObject["typePersona"] !== "1" && (
                ...
                >
                  ...
                  >
                    ...
                    {listOfOcupations.map((type) => (
                      <option value={type["idOcupation"]}>
                        {type["oCupation"]}
                      </option>
                    ))}
                  </select>
              ...
              {formObject["typePersona"] !== "1" && (
                <div
                  className={`${
                    toggledSidebar ? "w-1/2" : "w-1/4"
                  } mb-3 px-3 flex justify-start items-center `}
                >
                  <p className="input-label flex items-center leading-[2px] mr-2">
                    PEP
                    <div className="ml-3 w-1/4">
            ...
            <div className="my-4">
              ...
              <div className="flex flex-wrap content-start">
                ...
                >
                  ...
                  <select
                    defaultValue={formObject.cantidadReferencias}
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        cantidadReferencias: e.target.value,
                      });
                    }}
                    className="form-control"
                  >
              ...
              <div className="flex flex-wrap content-start">
                ...
                >
                  ...
                  <input
                    defaultValue={formObject.NombreReferencia1}
                    placeholder={"Escriba el nombre de la referencia"}
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        NombreReferencia1: e.target.value,
                      });
                    }}
                    type={"text"}
                    className="form-control"
                ...
                >
                  <p className="input-label">
                    Actividad Referencia - 1{" "}
                    <span className="text-primary font-bold">*</span>
                  </p>
                  <input
                    defaultValue={formObject.ActividadReferencia1}
                    placeholder={"Escriba la actividad de la referencia"}
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        ActividadReferencia1: e.target.value,
                      });
                    }}
                    type={"text"}
                    className="form-control"
                ...
                >
                  <p className="input-label">
                    Relación Cliente referencia - 1{" "}
                    <span className="text-primary font-bold">*</span>
                  </p>
                  <input
                    defaultValue={formObject.RelacionClienteReferencia1}
                    placeholder={"Escriba la relacion cliente referencia"}
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        RelacionClienteReferencia1: e.target.value,
                      });
                    }}
                    type={"text"}
                    className="form-control"
                ...
                >
                  ...
                  <input
                    defaultValue={formObject.TelefonoReferencia1}
                    placeholder={"Escriba el telefono de la referencia"}
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        TelefonoReferencia1: e.target.value,
                      });
                    }}
                    type={"text"}
                    className="form-control"
              ...
              {(formObject.cantidadReferencias === "2" ||
                formObject.cantidadReferencias === "3") && (
                <div className="flex flex-wrap content-start">
                  <div
                    className={`${
                      toggledSidebar ? "w-1/2" : "w-1/4"
                  ...
                  >
                    ...
                    <input
                      defaultValue={formObject.NombreReferencia2}
                      placeholder={"Escriba el nombre de la referencia"}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          NombreReferencia2: e.target.value,
                        });
                      }}
                      type={"text"}
                      className="form-control"
                  ...
                  >
                    <p className="input-label">
                      Actividad Referencia - 2{" "}
                      <span className="text-primary font-bold">*</span>
                    </p>
                    <input
                      defaultValue={formObject.ActividadReferencia2}
                      placeholder={"Escriba la actividad de la referencia"}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          ActividadReferencia2: e.target.value,
                        });
                      }}
                      type={"text"}
                      className="form-control"
                  ...
                  >
                    <p className="input-label">
                      Relación Cliente Referencia - 2{" "}
                      <span className="text-primary font-bold">*</span>
                    </p>
                    <input
                      defaultValue={formObject.RelacionClienteReferencia2}
                      placeholder={"Escriba la relacion cliente referencia"}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          RelacionClienteReferencia2: e.target.value,
                        });
                      }}
                      type={"text"}
                      className="form-control"
                  ...
                  >
                    ...
                    <input
                      defaultValue={formObject.TelefonoReferencia2}
                      placeholder={"Escriba el telefono de la referencia"}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          TelefonoReferencia2: e.target.value,
                        });
                      }}
                      type={"text"}
                      className="form-control"
              ...
              {formObject.cantidadReferencias === "3" && (
                <div className="flex flex-wrap content-start">
                  ...
                  >
                    ...
                    <input
                      defaultValue={formObject.NombreReferencia3}
                      placeholder={"Escriba el nombre de la referencia"}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          NombreReferencia3: e.target.value,
                        });
                      }}
                      type={"text"}
                      className="form-control"
                  ...
                  >
                    <p className="input-label">
                      Actividad Referencia - 3{" "}
                      <span className="text-primary font-bold">*</span>
                    </p>
                    <input
                      defaultValue={formObject.ActividadReferencia3}
                      placeholder={"Escriba la actividad de la referencia"}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          ActividadReferencia3: e.target.value,
                        });
                      }}
                      type={"text"}
                      className="form-control"
                  ...
                  >
                    <p className="input-label">
                      Relación Cliente Referencia - 3{" "}
                      <span className="text-primary font-bold">*</span>
                    </p>
                    <input
                      defaultValue={formObject.RelacionClienteReferencia3}
                      placeholder={"Escriba la relacion cliente referencia"}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          RelacionClienteReferencia3: e.target.value,
                        });
                      }}
                      type={"text"}
                      className="form-control"
                  ...
                  >
                    ...
                    <input
                      defaultValue={formObject.TelefonoReferencia3}
                      placeholder={"Escriba el telefono de la referencia"}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          TelefonoReferencia3: e.target.value,
                        });
                      }}
                      type={"text"}
                      className="form-control"
          ...
          {isPep && formObject["typePersona"] !== "1" && (
            <div className="my-4" ref={thirdRef}>
              ...
              <div className="flex flex-wrap content-start">
                ...
                >
                  ...
                  <input
                    defaultValue={formObject.RelacionCargoPep}
                    placeholder="Escriba la relacion/cargo"
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        RelacionCargoPep: e.target.value,
                      });
                    }}
                    type="text"
                    className="form-control"
                  />
                  {fieldsClientPEP.includes("RelacionCargoPep") && (
                    <ErrorText />
                  )}
                </div>
                <div
                ...
                >
                  <p className="input-label">
                    Desde <span className="text-primary font-bold">*</span>
                  </p>
                  <InputComponent
                    fromYear={1950}
                    toYear={moment().year().toString()}
                    customOnChange={setFromPEP}
                    daySelected={fromPEP}
                  />
                </div>
                <div
                ...
                >
                  <p className="input-label">
                    Hasta <span className="text-primary font-bold">*</span>
                  </p>
                  <InputComponent
                    fromYear={1950}
                    toYear={moment().year().toString()}
                    customOnChange={setUntilPEP}
                    daySelected={untilPEP}
                  />
                </div>
                <div
                  className={`${toggledSidebar ? "w-1/2" : "w-1/4"} mb-3 px-3`}
                >
                  ...
                  <input
                    defaultValue={formObject.ObservacionPep}
                    placeholder="Escriba la observacion PEP"
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        ObservacionPep: e.target.value,
                      });
                    }}
                    type="text"
                    className="form-control"
                ...
                >
                  ...
                  <input
                    defaultValue={formObject.OtrasActividadesComercPep}
                    placeholder="Escriba las otras actividades comerciales"
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        OtrasActividadesComercPep: e.target.value,
                      });
                    }}
                    type="text"
                    className="form-control"
                  />
                  {fieldsClientPEP.includes("OtrasActividadesComercPep") && (
                    <ErrorText />
                  )}
                </div>
                <div
                ...
                >
                  ...
                  <select
                    defaultValue={formObject.IdPerfilFinanciero}
                    value={formObject.IdPerfilFinanciero}
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        IdPerfilFinanciero: e.target.value,
                      });
                    }}
                    className={"form-control"}
                  >
                    <option value="">Seleccione el perfil financiero</option>
                    {listOfFinancialProfiles.map((type) => (
                      <option value={type["IdPerfil"]}>{type["Perfil"]}</option>
                    ))}
                  </select>
                </div>
                <div
                ...
                >
                  <p className="input-label flex items-center leading-[15px!important] ">
                    Poliza con prima anual (Mayor a 10.000){" "}
                    <span className="text-primary font-bold">*</span>
                    <div className="ml-3 w-1/4">
                      <div
                        onClick={() => {
          ...
          {!sameDataContractor && (
            <div className="my-4" ref={secondRef}>
              ...
              <div className="flex flex-wrap content-start relative h-fit">
                {!loadedIDFromAPI && (
                  <span className="w-full h-full absolute bg-white/40 backdrop-blur-sm bottom-0 left-0 z-10 flex flex-col justify-center items-center">
                    <AiFillLock size={70} color="#242424" />
                  </span>
                )}
                ...
                >
                  <p className="input-label">Tipo de Documento de Identidad</p>
                  <select
                    defaultValue={formObject.IdTipoDocumentoContratante}
                    value={formObject.IdTipoDocumentoContratante}
                    onChange={(e) => {
                      changeTypePersonContractor(e.target.value);
                    }}
                    className={"form-control"}
                  >
                    <option value="">
                    ...
                    {listOfTypeDocumentContractor.map((type) => (
                      <option value={type["IdTipoDocumento"]}>
                        {type["TipoDocumento"]}
                      </option>
                    ))}
                  </select>
                  {fieldsContractor.includes("IdTipoDocumentoContratante") && (
                    <ErrorText />
                  )}
                </div>
                <div
                ...
                >
                  ...
                  <input
                    defaultValue={formObject.CedulaRucContratante}
                    value={formObject.CedulaRucContratante}
                    placeholder="Ingrese el número de identificación"
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        CedulaRucContratante: e.target.value,
                      });
                    }}
                    type="text"
                    className={"form-control"}
                  />
                  {fieldsContractor.includes("CedulaRucContratante") && (
                    <ErrorText />
                  )}
                </div>

                {formObject["IdTipoDocumentoContratante"] !== "3" && (
                  <div
                    className={`${
                      toggledSidebar ? "w-1/2" : "w-1/4"
                    } mb-3 px-3`}
                  >
                    <p className={"input-label"}>Género</p>
                    <select
                      defaultValue={formObject.IdSexoContratante}
                      value={formObject.IdSexoContratante}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          IdSexoContratante: e.target.value,
                        });
                      }}
                      className={"form-control"}
                    >
                ...
                {formObject["IdTipoDocumentoContratante"] !== "3" && (
                  ...
                  >
                    ...
                    <select
                      defaultValue={formObject.IdEsoCivilContratante}
                      value={formObject.IdEsoCivilContratante}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          IdEsoCivilContratante: e.target.value,
                        });
                      }}
                      className={"form-control"}
                    >
                    ...
                    {fieldsContractor.includes("IdEsoCivilContratante") && (
                      <ErrorText />
                    )}
                  </div>
                )}
                ...
                {formObject["IdTipoDocumentoContratante"] !== "3" && (
                  ...
                  >
                    ...
                    <input
                      defaultValue={formObject.NombreContratante}
                      placeholder="Ingrese los nombres"
                      value={formObject.NombreContratante}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          NombreContratante: e.target.value,
                        });
                      }}
                      type="text"
                      className={"form-control"}
                    />
                    {fieldsContractor.includes("NombreContratante") && (
                      <ErrorText />
                    )}
                  </div>
                )}
                {formObject["IdTipoDocumentoContratante"] !== "3" && (
                  ...
                  >
                    ...
                    <input
                      defaultValue={formObject.ApellidoContratante}
                      placeholder="Ingrese los apellidos"
                      value={formObject.ApellidoContratante}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          ApellidoContratante: e.target.value,
                        });
                      }}
                      type="text"
                      className={"form-control"}
                    />
                    {fieldsContractor.includes("ApellidoContratante") && (
                      <ErrorText />
                    )}
                  </div>
                )}
                {formObject["IdSexoContratante"] === "1" &&
                  formObject["IdEsoCivilContratante"] !== "1" && (
                    <div
                      className={`${
                        toggledSidebar ? "w-1/2" : "w-1/4"
                      } mb-3 px-3`}
                    >
                      <p className={"input-label"}>Apellido de Casada</p>
                      <input
                        defaultValue={formObject.ApellidoCasadaContratante}
                        value={formObject.ApellidoCasadaContratante}
                        placeholder="Ingrese el apellido de casada"
                        onChange={(e) => {
                          setFormObject({
                            ...formObject,
                            ApellidoCasadaContratante: e.target.value,
                          });
                        }}
                        type="text"
                        className={"form-control"}
                ...
                >
                  ...
                  <input
                    defaultValue={formObject.EmailContratante}
                    placeholder="Ingrese el correo electrónico"
                    value={formObject.EmailContratante}
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                    ...
                    className={"form-control"}
                  />
                  {fieldsContractor.includes("EmailContratante") && (
                    <ErrorText />
                  )}
                </div>
                <div
                ...
                >
                  ...
                  <input
                    defaultValue={formObject.TelefonoContratante}
                    placeholder="Ingrese el número telefónico"
                    value={formObject.TelefonoContratante}
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        TelefonoContratante: e.target.value,
                      });
                    }}
                    type="phone"
                    className={"form-control"}
                ...
                {formObject["IdTipoDocumentoContratante"] === "3" && (
                  ...
                  >
                    ...
                    <input
                      defaultValue={formObject.NombreComercialContratante}
                      value={formObject["NombreComercialContratante"]}
                      placeholder="Ingrese el nombre comercial"
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          NombreComercialContratante: e.target.value,
                        });
                      }}
                      type="text"
                      className="form-control"
                    />
                    {fieldsContractorJuridic.includes(
                ...
                {formObject["IdTipoDocumentoContratante"] === "3" && (
                  ...
                  >
                    ...
                    <input
                      defaultValue={formObject.RepresentanteLegalContratante}
                      value={formObject["RepresentanteLegalContratante"]}
                      placeholder="Ingrese el representante legal"
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          RepresentanteLegalContratante: e.target.value,
                        });
                      }}
                      type="text"
                      className="form-control"
                ...
                {formObject["IdTipoDocumentoContratante"] === "3" && (
                  ...
                  >
                    <p className="input-label">Nombre Beneficiario</p>
                    <input
                      defaultValue={formObject.NombreBeneficiarioContratante}
                      value={formObject["NombreBeneficiarioContratante"]}
                      placeholder="Ingrese el nombre del beneficiario"
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          NombreBeneficiarioContratante: e.target.value,
                        });
                      }}
                      type="text"
                      className="form-control"
                  ...
                  >
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          IdentificacionBeneficiarioContratante: e.target.value,
                        });
                      }}
                      type="text"
                      className="form-control"
                ...
                {formObject["IdTipoDocumentoContratante"] !== "3" && (
                  ...
                  >
                    ...
                    <select
                      defaultValue={formObject.IdNacionalidadContratante}
                      value={formObject.IdNacionalidadContratante}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          IdNacionalidadContratante: e.target.value,
                        });
                      }}
                      className={"form-control"}
                    >
                ...
                {formObject["IdTipoDocumentoContratante"] !== "3" && (
                  ...
                  >
                    <p className={"input-label"}>Profesión</p>
                    <select
                      defaultValue={formObject.IdProfesionContratante}
                      value={formObject.IdProfesionContratante}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          IdProfesionContratante: e.target.value,
                        });
                      }}
                      className={"form-control"}
                    >
                ...
                {formObject["IdTipoDocumentoContratante"] !== "3" && (
                  ...
                  >
                    <p className={"input-label"}>Ocupación</p>
                    <select
                      defaultValue={formObject.IdOcupacionContratante}
                      value={formObject.IdOcupacionContratante}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          IdOcupacionContratante: e.target.value,
                        });
                      }}
                      className={"form-control"}
                    >
          ...
          {isPepContractor && (
            <div className="my-4">
              <p className={`title-section text-slate-900`}>
                Referencias Personales - Contratante
              </p>
              <div className="flex flex-wrap content-start">
                <div
                  className={`${toggledSidebar ? "w-1/2" : "w-1/4"} mb-3 px-3`}
                >
                  ...
                  <select
                    defaultValue={formObject.cantidadReferenciasContratante}
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        cantidadReferenciasContratante: e.target.value,
                      });
                    }}
                    className="form-control"
                  >
              ...
              <div className="flex flex-wrap content-start">
                ...
                >
                  ...
                  <input
                    defaultValue={formObject.NombreReferencia1Contratante}
                    placeholder={"Escriba el nombre de la referencia"}
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        NombreReferencia1Contratante: e.target.value,
                      });
                    }}
                    type={"text"}
                    className="form-control"
                ...
                >
                  ...
                  <input
                    defaultValue={formObject.ActividadReferencia1Contratante}
                    placeholder={"Escriba la actividad de la referencia"}
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        ActividadReferencia1Contratante: e.target.value,
                      });
                    }}
                    type={"text"}
                    className="form-control"
                ...
                >
                  ...
                  <input
                    defaultValue={
                      formObject.RelacionClienteReferencia1Contratante
                    }
                    placeholder={"Escriba la relacion cliente referencia"}
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        RelacionClienteReferencia1Contratante: e.target.value,
                      });
                    }}
                    type={"text"}
                    className="form-control"
                ...
                >
                  ...
                  <input
                    defaultValue={formObject.TelefonoReferencia1Contratante}
                    placeholder={"Escriba el telefono de la referencia"}
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        TelefonoReferencia1Contratante: e.target.value,
                      });
                    }}
                    type={"text"}
                    className="form-control"
              ...
              {(formObject.cantidadReferenciasContratante === "2" ||
                formObject.cantidadReferenciasContratante === "3") && (
                <div className="flex flex-wrap content-start">
                  <div
                    className={`${
                      toggledSidebar ? "w-1/2" : "w-1/4"
                  ...
                  >
                    ...
                    <input
                      defaultValue={formObject.NombreReferencia2Contratante}
                      placeholder={"Escriba el nombre de la referencia"}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          NombreReferencia2Contratante: e.target.value,
                        });
                      }}
                      type={"text"}
                      className="form-control"
                  ...
                  >
                    ...
                    <input
                      defaultValue={formObject.ActividadReferencia2Contratante}
                      placeholder={"Escriba la actividad de la referencia"}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          ActividadReferencia2Contratante: e.target.value,
                        });
                      }}
                      type={"text"}
                      className="form-control"
                  ...
                  >
                    ...
                    <input
                      defaultValue={
                        formObject.RelacionClienteReferencia2Contratante
                      }
                      placeholder={"Escriba la relacion cliente referencia"}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          RelacionClienteReferencia2Contratante: e.target.value,
                        });
                      }}
                      type={"text"}
                      className="form-control"
                  ...
                  >
                    ...
                    <input
                      defaultValue={formObject.TelefonoReferencia2Contratante}
                      placeholder={"Escriba el telefono de la referencia"}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          TelefonoReferencia2Contratante: e.target.value,
                        });
                      }}
                      type={"text"}
                      className="form-control"
              ...
              {formObject.cantidadReferenciasContratante === "3" && (
                <div className="flex flex-wrap content-start">
                  ...
                  >
                    ...
                    <input
                      defaultValue={formObject.NombreReferencia3Contratante}
                      placeholder={"Escriba el nombre de la referencia"}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          NombreReferencia3Contratante: e.target.value,
                        });
                      }}
                      type={"text"}
                      className="form-control"
                  ...
                  >
                    ...
                    <input
                      defaultValue={formObject.ActividadReferencia3Contratante}
                      placeholder={"Escriba la actividad de la referencia"}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          ActividadReferencia3Contratante: e.target.value,
                        });
                      }}
                      type={"text"}
                      className="form-control"
                  ...
                  >
                    ...
                    <input
                      defaultValue={
                        formObject.RelacionClienteReferencia3Contratante
                      }
                      placeholder={"Escriba la relacion cliente referencia"}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          RelacionClienteReferencia3Contratante: e.target.value,
                        });
                      }}
                      type={"text"}
                      className="form-control"
                  ...
                  >
                    ...
                    <input
                      defaultValue={formObject.TelefonoReferencia3Contratante}
                      placeholder={"Escriba el telefono de la referencia"}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          TelefonoReferencia3Contratante: e.target.value,
                        });
                      }}
                      type={"text"}
                      className="form-control"
              ...
              <div className="my-4" ref={thirdRef}>
                ...
                <div className="flex flex-wrap content-start">
                  ...
                  >
                    ...
                    <input
                      defaultValue={formObject.RelacionCargoPepContratante}
                      placeholder="Escriba la relacion/cargo"
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          RelacionCargoPepContratante: e.target.value,
                        });
                      }}
                      type="text"
                      className="form-control"
                    />
                    {fieldsContractorPEP.includes(
                      "RelacionCargoPepContratante"
                    ) && <ErrorText />}
                  </div>
                  <div
                    className={`${
                  ...
                  >
                    ...
                    <InputComponent
                      fromYear={1950}
                      toYear={moment().year().toString()}
                      customOnChange={setFromPEPContractor}
                      daySelected={fromPEPContractor}
                    />
                  </div>
                  <div
                    className={`${
                  ...
                  >
                    ...
                    <InputComponent
                      fromYear={1950}
                      toYear={moment().year().toString()}
                      customOnChange={setUntilPEPContractor}
                      daySelected={untilPEPContractor}
                    />
                  </div>
                  <div
                    className={`${
                  ...
                  >
                    ...
                    <input
                      value={formObject.ObservacionPepContratante}
                      placeholder="Escriba la observacion PEP"
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          ObservacionPepContratante: e.target.value,
                        });
                      }}
                      type="text"
                      className="form-control"
                  ...
                  >
                    ...
                    <input
                      defaultValue={
                        formObject.OtrasActividadesComercPepContratante
                      }
                      placeholder="Escriba las otras actividades comerciales"
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          OtrasActividadesComercPepContratante: e.target.value,
                        });
                      }}
                      type="text"
                      className="form-control"
                    />
                    {fieldsContractorPEP.includes(
                      "OtrasActividadesComercPepContratante"
                    ) && <ErrorText />}
                  </div>
                  <div
                    className={`${
                  ...
                  >
                    <p className={"input-label"}>Perfil Financiero</p>
                    <select
                      defaultValue={formObject.IdPerfilFinancieroContratante}
                      value={formObject.IdPerfilFinancieroContratante}
                      onChange={(e) => {
                        setFormObject({
                          ...formObject,
                          IdPerfilFinancieroContratante: e.target.value,
                        });
                      }}
                      className={"form-control"}
                    >
          ...
          <div className="my-4" ref={thirdRef}>
            ...
            <div className="flex flex-wrap content-start relative h-fit">
              ...
              >
                <p className="input-label">
                  Provincia <span className="text-primary font-bold">*</span>
                </p>
                <select
                  defaultValue={formObject["IdProvincia"]}
                  onChange={(e) => {
                    getDataFromValue(+e.target.value, "provincia");
                  }}
                  className="form-control"
                >
                  <option value="">Seleccione la provincia</option>
                  {listOfProvincias.map((type) => (
              ...
              >
                <p className="input-label">
                  Distrito <span className="text-primary font-bold">*</span>
                </p>
                <select
                  defaultValue={formObject["IdDistrito"]}
                  onChange={(e) => {
                    getDataFromValue(+e.target.value, "distrito");
                  }}
                  className="form-control"
                >
                  <option value="">Seleccione el distrito</option>
                  {listOfDistritos.map((type) => (
                    <option value={type["id"]}>{type["descripcion"]}</option>
                  ))}
                </select>
              ...
              >
                ...
                <select
                  defaultValue={formObject["IdCorregimiento"]}
                  onChange={(e) => {
                    getDataFromValue(+e.target.value, "corregimiento");
                  }}
                  className="form-control"
                >
                  <option value="">Seleccione el distrito</option>
                  {listOfCorregimientos.map((type) => (
                    <option value={type["id"]}>{type["descripcion"]}</option>
                  ))}
                </select>
              ...
              >
                <p className="input-label">
                  Urbanización <span className="text-primary font-bold">*</span>
                </p>
                <select
                  defaultValue={formObject["Urbanizacion"]}
                  onChange={(e) => {
                    setFormObject({
                      ...formObject,
              ...
              >
                ...
                <input
                  value={formObject["Calle"]}
                  placeholder={"Escriba la calle"}
                  onChange={(e) => {
                    setFormObject({ ...formObject, Calle: e.target.value });
                  }}
                  type="text"
                  className="form-control"
                />
              ...
              {formObject["typePersona"] !== "1" && (
                ...
                >
                  ...
                  <input
                    value={formObject["ApartamentoCasa"]}
                    placeholder={"Escriba el apartamento / casa"}
                    onChange={(e) => {
                      setFormObject({
                        ...formObject,
                        ApartamentoCasa: e.target.value,
                      });
                    }}
                    type="text"
                    className="form-control"
              ...
              >
                ...
                <input
                  value={formObject["Direccion"]}
                  placeholder={"Escriba la dirección"}
                  onChange={(e) => {
                    setFormObject({ ...formObject, Direccion: e.target.value });
                  }}
                  type="text"
                  className="form-control"
                />
          ...
          <div className="my-4" ref={fourthRef}>
            ...
            <div className="flex flex-wrap content-start relative">
              ...
              >
                ...
                <input
                  defaultValue={formObject["NombreConductorAdicional"]}
                  placeholder={"Escriba los nombres del conductor adicional"}
                  onChange={(e) => {
                    setFormObject({
                      ...formObject,
                      NombreConductorAdicional: e.target.value,
                    });
                  }}
                  type={"text"}
                  className="form-control"
              ...
              >
                ...
                <input
                  defaultValue={formObject["ApellidoConductorAdic"]}
                  placeholder={"Escriba los apellidos del conductor adicional"}
                  onChange={(e) => {
                    setFormObject({
                      ...formObject,
                      ApellidoConductorAdic: e.target.value,
                    });
                  }}
                  type={"text"}
                  className="form-control"
              ...
              >
                ...
                <input
                  defaultValue={formObject["IdentificacionAdic"]}
                  placeholder={
                    "Escriba el identificacion del conductor adicional"
                  }
                  onChange={(e) => {
                    setFormObject({
                      ...formObject,
                      IdentificacionAdic: e.target.value,
                    });
                  }}
                  type={"text"}
                  className="form-control"
          ...
          <div className="my-4" ref={fourthRef}>
            ...
            <div className="flex flex-wrap content-start relative">
              ...
              >
                <p className="input-label">
                  Marca <span className="text-primary font-bold">*</span>
                </p>
                <select
                  value={formObject["marcaid"]}
                  onChange={(e) => {
                    getDataFromValue(+e.target.value, "model");
                  }}
                  className="form-control"
                >
                  <option value="">Seleccione la marca</option>
                  {listOfVehiculeBrands.map((type) => (
                    <option value={type["idMarca"]}>{type["marca"]}</option>
                  ))}
                </select>
              ...
              >
                <p className="input-label">
                  Modelo <span className="text-primary font-bold">*</span>
                </p>
                <select
                  value={formObject["modeloid"]}
                  onChange={(e) => {
                    setFormObject({ ...formObject, modeloid: +e.target.value });
                  }}
                  className="form-control"
                >
                  <option value="">Seleccione el modelo</option>
                  {listOfVehiculeModels.map((type) => (
                    <option value={type["idModelo"]}>{type["modelo"]}</option>
                  ))}
                </select>
              ...
              >
                <p className="input-label">
                  Año <span className="text-primary font-bold">*</span>
                </p>
                <select
                  value={formObject["Ano"]}
                  onChange={(e) => {
                    setFormObject({ ...formObject, Ano: e.target.value });
                  }}
                  className="form-control"
                >
              ...
              >
                <p className="input-label">
                  Remolque <span className="text-primary font-bold">*</span>
                </p>
                <div className="flex justify-between items-center">
                  <p className="input-label flex items-center leading-[2px] mr-2">
                    Si
              ...
              >
                ...
                <NumericFormat
                  value={formObject["ValorVehiculo"]}
                  className="form-control"
                  onChange={(e) => {
                    setFormObject({
                      ...formObject,
              ...
              >
                ...
                <input
                  value={formObject["numeroMotor"]}
                  placeholder={"Ingrese el número del motor"}
                  onChange={(e) => {
                    setFormObject({
                      ...formObject,
                      numeroMotor: e.target.value,
                    });
                  }}
                  type={"text"}
                  className="form-control"
              ...
              >
                ...
                <input
                  value={formObject["numeroChasis"]}
                  placeholder={"Ingrese el número de chasis"}
                  onChange={(e) => {
                    setFormObject({
                      ...formObject,
                      numeroChasis: e.target.value,
                    });
                  }}
                  type={"text"}
                  className="form-control"
              ...
              >
                <p className="input-label">
                  Placa <span className="text-primary font-bold">*</span>
                </p>
                <InputMask
                  type="text"
                  maskChar={null}
                  ...
                  onChange={(e) =>
                    setFormObject({ ...formObject, Placa: e.target.value })
                  }
                />
                {formObject["Placa"] && formObject["Placa"].length < 6 && (
                  <p className="text-[0.8rem] mt-2 text-red-700">
                    La placa debe ser mayor a 5 digitos
                  </p>
                )}
              ...
              >
                ...
                <input
                  value={formObject["Color"]}
                  placeholder={"Escriba el color"}
                  onChange={(e) => {
                    setFormObject({ ...formObject, Color: e.target.value });
                  }}
                  type={"text"}
                  className="form-control"
                />
              ...
              >
                ...
                <input
                  value={formObject["CantidadPasajeros"]}
                  placeholder={"Escriba la cantidad de pasajeros"}
                  onChange={(e) => {
                    setFormObject({
                      ...formObject,
              ...
              >
                <p className="input-label">
                  Registro Vehicular (pdf, jpg, png)
                </p>
                <input
                  placeholder={"Seleccionar"}
                  onChange={selectFiles}
  ...
  function handleDateFormat(prop) {
    let returnedDate = prop !== undefined ? prop.toString() : "";
    return returnedDate;
  }
  ...
  function handleValidationFunction() {
    let postObject = {
      ...
      ApellidoCasadaContratante:
        handleCopyPasteDataContractor(
          formObject["ApellidoCasadaContratante"],
          formObject["marriedLastName"]
        ) ?? "",
      FechaNacimientoContratante:
        handleCopyPasteDataContractor(
          dateBirthClient,
          dateBirthClientContractor
        ) ?? "",
      CedulaRucContratante:
        handleCopyPasteDataContractor(
          formObject["CedulaRucContratante"],
          formObject["numberId"]
        ) ?? "",
      IdTipoDocumentoContratante: sameDataContractor
        ? "3"
        : formObject["IdTipoDocumentoContratante"],
      IdTipoPersonaContratante: formObject["IdTipoPersonaContratante"] ?? "",
      IdSexoContratante: handleCopyPasteDataContractor(
        formObject["IdSexoContratante"],
        formObject["idsexo"]
      ),
      EmailContratante:
      ...
      IdNacionalidadContratante:
        handleCopyPasteDataContractor(
          formObject["IdNacionalidadContratante"],
          formObject["IdPais"]
        ) ?? "",
      IdEsoCivilContratante:
        handleCopyPasteDataContractor(
          formObject["IdEsoCivilContratante"],
          formObject["idestadocivil"]
        ) ?? "",
      IdProfesionContratante:
        handleCopyPasteDataContractor(
          formObject["IdProfesionContratante"],
          formObject["IdProfesion"]
        ) ?? "",
      IdOcupacionContratante:
        handleCopyPasteDataContractor(
          formObject["IdOcupacionContratante"],
          formObject["IdOcupacion"]
        ) ?? "",
      PolizaAnualMayorLimiteContratante: policeAnualPrime ? 1 : 2,
      PolizaAnualMayorLimiteContratanteContratante: policeAnualPrimeContractor
        ? 1
        : 2,
      IdPerfilFinanciero: formObject["IdPerfilFinanciero"] ?? "",
      IdPerfilFinancieroContratante:
        formObject["IdPerfilFinancieroContratante"] ?? "",
      ...
      RelacionCargoPepContratante:
        handleCopyPasteDataContractor(
          formObject["RelacionCargoPepContratante"],
          formObject["RelacionCargoPep"]
        ) ?? "",
      DesdePepContratante:
        handleCopyPasteDataContractor(fromPEPContractor, fromPEP) ?? "",
      HastaPepContratante:
        handleCopyPasteDataContractor(untilPEPContractor, untilPEP) ?? "",
      ObservacionPepContratante:
        handleCopyPasteDataContractor(
          formObject["ObservacionPepContratante"],
          formObject["ObservacionPep"]
        ) ?? "",
      OtrasActividadesComercPepContratante:
        handleCopyPasteDataContractor(
          formObject["OtrasActividadesComercPepContratante"],
          formObject["OtrasActividadesComercPep"]
        ) ?? "",

      NombreReferencia1: formObject["NombreReferencia1"] ?? "",
      ActividadReferencia1: formObject["ActividadReferencia1"] ?? "",
      RelacionClienteReferencia1:
        formObject["RelacionClienteReferencia1"] ?? "",
      ...
      RelacionClienteReferencia2:
        formObject["RelacionClienteReferencia2"] ?? "",
      ...
      RelacionClienteReferencia3:
        formObject["RelacionClienteReferencia3"] ?? "",
      TelefonoReferencia3: formObject["TelefonoReferencia3"] ?? "",

      NombreReferencia1Contratante:
        handleCopyPasteDataContractor(
          formObject["NombreReferencia1Contratante"],
          formObject["NombreReferencia1"]
        ) ?? "",
      ActividadReferencia1Contratante:
        handleCopyPasteDataContractor(
          formObject["ActividadReferencia1Contratante"],
          formObject["ActividadReferencia1"]
        ) ?? "",
      RelacionClienteReferencia1Contratante:
        handleCopyPasteDataContractor(
          formObject["RelacionClienteReferencia1Contratante"],
          formObject["RelacionClienteReferencia1"]
        ) ?? "",
      TelefonoReferencia1Contratante:
        handleCopyPasteDataContractor(
          formObject["TelefonoReferencia1Contratante"],
          formObject["TelefonoReferencia1"]
        ) ?? "",

      NombreReferencia2Contratante:
        handleCopyPasteDataContractor(
          formObject["NombreReferencia2Contratante"],
          formObject["NombreReferencia2"]
        ) ?? "",
      ActividadReferencia2Contratante:
        handleCopyPasteDataContractor(
          formObject["ActividadReferencia2Contratante"],
          formObject["ActividadReferencia2"]
        ) ?? "",
      RelacionClienteReferencia2Contratante:
        handleCopyPasteDataContractor(
          formObject["RelacionClienteReferencia2Contratante"],
          formObject["RelacionClienteReferencia2"]
        ) ?? "",
      TelefonoReferencia2Contratante:
        handleCopyPasteDataContractor(
          formObject["TelefonoReferencia2Contratante"],
          formObject["TelefonoReferencia2"]
        ) ?? "",

      NombreReferencia3Contratante:
        handleCopyPasteDataContractor(
          formObject["NombreReferencia3Contratante"],
          formObject["NombreReferencia3"]
        ) ?? "",
      ActividadReferencia3Contratante:
        handleCopyPasteDataContractor(
          formObject["ActividadReferencia3Contratante"],
          formObject["ActividadReferencia3"]
        ) ?? "",
      RelacionClienteReferencia3Contratante:
        handleCopyPasteDataContractor(
          formObject["RelacionClienteReferencia3Contratante"],
          formObject["RelacionClienteReferencia3"]
        ) ?? "",
      TelefonoReferencia3Contratante:
        handleCopyPasteDataContractor(
          formObject["TelefonoReferencia3Contratante"],
          formObject["TelefonoReferencia3"]
        ) ?? "",
    };
    return postObject;
  }
  ...
  function manageQuoterRegister() {
    ...
    let _fieldsClient = validators.validateObjectWithList(
      client_natural_list,
      object
    );
    let _fieldsContractor = validators.validateObjectWithList(
      contractor_natural_list,
      object
    );

    if (formObject["typePersona"] === "2") setFieldsClient(_fieldsClient);
    if (formObject["IdTipoDocumentoContratante"] !== "3")
      setFieldsContractor(_fieldsContractor);

    let _fieldsClientJuridic = validators.validateObjectWithList(
      client_juridic_list,
      object
    );
    let _fieldsContractorJuridic = validators.validateObjectWithList(
      contractor_juridic_list,
      object
    );

    if (formObject["typePersona"] !== "2")
      setFieldsClientJuridic(_fieldsClientJuridic);
    if (formObject["IdTipoDocumentoContratante"] === "3")
      setFieldsContractorJuridic(_fieldsContractorJuridic);

    let _fieldsClientPEP = validators.validateObjectWithList(pep_list, object);
    let _fieldsContractorPEP = validators.validateObjectWithList(
      pep_juridic_list,
      object
    );
